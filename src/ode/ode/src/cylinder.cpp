...
//--------------------------------------------------------
//--------------------------------------------------------
// Main Cylinder-Cylinder Collision Detection Program
//--------------------------------------------------------
//--------------------------------------------------------
//********************************************************
//////////////////////////////////////////////////////////

int sCylinderCylinderData::performCollisionChecking()
{
  ...
  // Two (fast exit) Axis Separating Tests before entering more complex tests
  //------------------------------------------------------------------------------
  dReal ch1, ch2, sr1, sr2;
  initProjectedLengths(ch1, ch2, sr1, sr2);
  const dReal hSum1 = m_fH1 + dFabs(ch2) + sr2;
  const dReal hSum2 = dFabs(ch1) + sr1 + m_fH2;
  if (dFabs(m_fDelta1) > hSum1 || dFabs(m_fDelta2) > hSum2) {
    dMessage(1," Separating Axes Tests\n");
    return 0;
  }
  ...
  // From now on, axes are not parallel
  //-----------------------------------

  dMessage(1,"Axes are not parallel : det = %f", m_fDet);
  ...
  if (centersPenetration(NULL, NULL))
    return m_nNumberOfContacts;
  ...
  //-------------------------
  // If yes, we must handle the special case of a cap containing (a part of) a generator
  //------------------------------------------------------------------------------------
  ...
  if (cPlus < thresholdCosinus1) {
    ...
    // Now, the bounded cylinders are PROBABLY intersecting in the following way:
    ...
    // The axes beeing orthogonal, we proceed a specific test
    //------------------------------------------------------
    ...
    const bool B1 = rho1 > 0.0;
    const bool B2 = rho2 > 0.0;
    bool exit = false;

    if (d12 <= d21) {
      if (B2) {
        orthogonalAxesIntersectionTest(A1, m_mRotation1, m_mRotation2, m_fRadius1, m_fRadius2, m_fH1, m_fH2, m_fDelta1, m_fDelta2, t1, t2,
          rho1, rho2, d12, d21, epsilon1, epsilon2, REAL(1.0), exit);
        if (exit)
          return m_nNumberOfContacts;
      }// end if B2
      if (B1) {
        dMessage(1,"Orthogonal axes : symmetrized test started\n");
        dNegateVector3(m_vW);
        const dVector3 A2 = { m_vCenter2[0] + t2 * m_mRotation2[2], m_vCenter2[1] + t2 * m_mRotation2[6], m_vCenter2[2] + t2 * m_mRotation2[10] };
        orthogonalAxesIntersectionTest(A2,m_mRotation2, m_mRotation1, m_fRadius2, m_fRadius1, m_fH2, m_fH1, -m_fDelta2, -m_fDelta1, t2, t1,
          rho2, rho1, d21, d12, -epsilon2, -epsilon1, REAL(-1.0), exit);
        if (exit)
          return m_nNumberOfContacts;
      }// end if B1
    } else { // end if (d12 <= d21)
    ...
    return m_nNumberOfContacts;//possibly no Collision
  ...
  // We test the upper/lower cap of the first cylinder against the upper/lower cap of the other
  //--------------------------------------------------------------------------------------------
  ...
  dMessage(1,"Entering CAG test");

  capAgainstGeneratorFullTest(h1n1, h2n2, t1, t2, ch1, ch2, dcac);
