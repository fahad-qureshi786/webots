...
#include "odemath.h"
#include "objects.h"
#include "fluid_dynamics/objects_fluid_dynamics.h"
#include "joints/joints.h"
#include "fluid_dynamics/immersion_link/immersion_link.h"
#include "step.h"
#include "quickstep.h"
#include "util.h"
#include "odetls.h"
...
dxBody *dBodyCreate_ST (dxWorld *w)
{
    dAASSERT (w);
    dxBody *b = new dxBody(w);
    b->firstjoint = 0;
    b->firstimmersionlink = 0;
    b->flags = 0;
    b->geom = 0;
...
{
#ifdef ODE_MT
    return dBodyCreate_MT(w, &dBodyCreate_ST);
#else
    return dBodyCreate_ST(w);
#endif
}

void dBodyRemovedAttachedImmersionLink(dxBody *b) {
  dxImmersionLink *nexti, *i = b->firstimmersionlink;
  while (i) {
    nexti = (dxImmersionLink *) i->nextBodyImmersionLink;
    dImmersionLinkDestroy(i);
    i = nexti;
  }
  b->firstimmersionlink = 0;
}

void dBodyDestroy_ST (dxBody *b)
{
    dAASSERT (b);
    ...
    // removed the attached immersion links
    dBodyRemovedAttachedImmersionLink(b);
...
{
#ifdef ODE_MT
    dBodyDestroy_MT(b, &dBodyDestroy_ST);
#else
    dBodyDestroy_ST(b);
#endif
}
...
int dJointGroupGetCount(dJointGroupID group)
{
    return group->getJointCount();
}
...
dxWorld * dWorldCreate()
{
    dxWorld* w;
#ifdef ODE_MT
     w = dWorldCreate_MT(&dWorldCreate_ST);
#else
     w = dWorldCreate_ST();
#endif
return w;
...
void dWorldDestroy_ST (dxWorld *w)
{
    ...
    dxFluid *nextf, *f = w->firstfluid;
    while (f) {
        nextf = (dxFluid*) f->next;
        dFluidDestroy(f); // calling here dBodyDestroy for correct destroying! (i.e. the average buffers)
        f = nextf;
    }
...
{
#ifdef ODE_MT
    return dWorldDestroy_MT(w, &dWorldDestroy_ST);
#else
    return dWorldDestroy_ST(w);
#endif
}
